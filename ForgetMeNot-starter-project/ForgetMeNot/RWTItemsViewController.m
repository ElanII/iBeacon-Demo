...
//@interface RWTItemsViewController () <UITableViewDataSource, UITableViewDelegate>   CLLocationManagerDelegate is a protocol the class conforms to
...
- (void)viewDidLoad {
    ...
    self.locationManager.delegate = self; //This simply instructs the location manager that the class wants to receive its delegate method calls.
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"Add"]) {
        ...
        [addItemViewController setItemAddedCompletion:^(RWTItem *newItem) {
            ...
            [self persistItems]; //saves new item in NSUserdefaults
        }];
    }
}
...
//Now you need a method to begin monitoring a given item. VV (below)
- (void)startMonitoringItem:(RWTItem *)item {
    CLBeaconRegion *beaconRegion = [self beaconRegionWithItem:item];
    if ([CLLocationManager authorizationStatus] == kCLAuthorizationStatusAuthorizedAlways) {
    [self.locationManager startMonitoringForRegion:beaconRegion];                  //start monitoring for beacons   - runs in background, regardless of whether the app is running
    [self.locationManager startRangingBeaconsInRegion:beaconRegion];           //ranging means youre within, discovering beacons and determining their distance. Happens only while the app is running.
    //CLLocationManager locationManager needs an object of type CLBeaconRegion
    }
}
...
// Error handling prints out to NSLog
...
// This is called when iBeacons come within range, move out of range, or when the range of an iBeacon changes
    ...
    for (CLBeacon *beacon in beacons) {
        for (RWTItem *item in self.items) {
            if ([item isEqualToCLBeacon:beacon]) {
                                                    // Determine if item is waequal to ranged beacon
                                                    // note CLBeaconRegion region is untouched
                item.lastSeenBeacon = beacon;  // sets property CLBeacon lastSeenBeacon in RWTItem.h
